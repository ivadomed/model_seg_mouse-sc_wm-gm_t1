...
if __name__ == '__main__':
    #Get the list of all full volume (not the slice extracted) from the conversion dictionnary
    f=open(conv_dict_path)
    conv_dict = json.load(f)
    list_3d_files = []
    for element in conv_dict:
        if ('slice' not in element) and ('T1w.nii.gz' in element):
    ...
    for element in list_3d_files:
        ...
        if(len([k for k in seg_files if seg_file_name in str(k)])!=0): 
